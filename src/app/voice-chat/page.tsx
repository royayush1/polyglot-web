...
// 1) Define both codes per language:
const languages = [
...
function useVoiceFor(ttsTag: string) {
  // hook to pick the matching SpeechSynthesisVoice
  const [voice, setVoice] = useState<SpeechSynthesisVoice | null>(null);

  // once on mount and when ttsTag changes
  useState(() => {
...
export default function VoiceChatPage() {
  // 2) Track selection, history, modes, and loading
  ...
  // 4) When recording stops: transcribe, chat, and speak
  const onStop = async () => {
    setMode('processing');
    setLoading(true);
    ...
    // Speak reply with full BCP-47 tag and selected voice
    const utter = new SpeechSynthesisUtterance(reply);
    utter.lang = sel.tts;            // e.g. "fr-FR"
  ...
  return (
    <div className='w-screen h-screen bg-blue-300 p-6'>
      <div className="max-w-xl mx-auto flex flex-col items-center space-y-4 border-4 border-black rounded bg-white p-4">
        <h1 className="text-2xl font-bold text-[#035A9D]">Voice Chat Tutor</h1>

        {/* 5) Language selector */}
        <select
          value={sel.stt}
          onChange={e => setSel(languages.find(l => l.stt === e.target.value)!)}
        ...
        >
          {mode === 'processing'
            ? <BounceLoader loading size={50} color='#57B9FF'/>
        ...
        <div className="h-64 overflow-y-auto border p-2 rounded w-full space-y-2 text-black">
          {msgs.map((m,i) => (
            <div key={i} className={m.role==='user'?'text-right':'text-left'}>
              <span className={`inline-block p-2 rounded ${m.role==='user'?'bg-green-200':'bg-blue-200'}`}>
                {m.content}
              </span>
            </div>
          ))}
